...
export function getContacts(req, res) {
  UserService.getContacts(req, function (err, contacts) {
    if (err)
...
export function createContact(req, res) {
  UserService.createContact(req, function (err, user) {
    if (err)
...
export function deleteContact(req, res) {
  UserService.deleteContact(req, function (err, user) {
    if (err)
      return handleError(res, err);
    return res.status(200).send(user);
  });
}
//Get all the chats done by the particular user
export function getRooms(req, res) {
  UserService.getRooms(req, function (err, rooms) {
    if (err)
      return handleError(res, err, 404);
    return res.status(200).send(rooms);
  });
}
//Start a new chat for the particular user
// export function addRoom(req, res) {
//   UserService.addRoom(req, function (err, user) {
//     if (err)
//       return handleError(res, err);
//     return res.status(200).send(user);
//   });
...
export function deleteRoom(req, res) {
  UserService.deleteRoom(req, function (err, user) {
    if (err)
      return handleError(res, err);
    return res.status(200).send(user);
  });
